...
use demo_db::{database::DbConn, Database};
...
impl Context {
    ...
    pub fn db(self) -> impl Filter<Extract = (DbConn,), Error = Rejection> + Clone {
        self.filter().and_then(move |ctx: Context| async move {
            Ok::<DbConn, Rejection>(ctx._db_conn())
        })
    }
